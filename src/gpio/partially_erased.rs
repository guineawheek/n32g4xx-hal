...
impl<const P: char, MODE> PartiallyErasedPin<P, Output<MODE>> {
    ...
    pub fn set_high(&mut self) {
        ...
        unsafe { (*gpiox::<P>()).pbsc().write(|w| w.bits(1 << self.i)) }
    }
    ...
    pub fn set_low(&mut self) {
        // NOTE(unsafe) atomic write to a stateless register
        unsafe { (*gpiox::<P>()).pbsc().write(|w| w.bits(1 << (self.i + 16))) }
    }
    ...
    pub fn is_set_low(&self) -> bool {
        ...
        unsafe { (*gpiox::<P>()).pod().read().bits() & (1 << self.i) == 0 }
    }
    ...
    MODE: marker::Readable,
{
    ...
    pub fn is_low(&self) -> bool {
        // NOTE(unsafe) atomic read with no side effects
        unsafe { (*gpiox::<P>()).pid().read().bits() & (1 << self.i) == 0 }
    }
}
